...
@click.option('--table_map_file', '-tm',
              ...
              required=True)
@click.option('--nx', '-nx',
              prompt='Number of voxels in X dimension of tomogram',
              type=int,
              required=True)
@click.option('--ny', '-ny',
              prompt='Number of voxels in Y dimension of tomogram',
              type=int,
              required=True)
@click.option('--nz', '-nz',
              prompt='Number of voxels in Z dimension of tomogram',
              type=int,
              required=True)
@click.option('--output_star_file', '-o',
              ...
              type=click.Path(),
              required=True)
def cli(input_table_file, table_map_file, output_star_file, nx, ny, nz):
    ...
    # prep normalisation constants
    n_xyz = {'x': nx, 'y': ny, 'z': nz}

    # extract xyz into dict with relion style headings and normalise coords
    for axis in ('x', 'y', 'z'):
        heading = f'rlnCoordinate{axis.upper()}'
        shift_axis = f'd{axis}'
        data[heading] = (table[axis] + table[shift_axis]) / n_xyz[axis]
